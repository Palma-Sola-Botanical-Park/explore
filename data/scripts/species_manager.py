...
<style>
  ...
  .vf-actions button.on { color:#fff; border-color:transparent; }
...
<div class="vf-wrap">
  ...
  <button id="vf-go" class="pub-btn airevise" style="display:none" onclick="vfVerify()">🔎 Ask Claude to verify</button>

  <div id="vf-status"></div>
  <div id="vf-results"></div>
  <div id="vf-applyrow" style="display:none; margin-top:14px;">
    <button class="pub-btn preview" onclick="vfApply()">💾 Apply decisions</button>
    <span style="color:#777; font-size:13px; margin-left:8px;">(writes accepted + overridden fields; rejects are left as-is)</span>
  </div>
</div>
...
<script>
  ...
  async function vfVerify(){
    ...
    } catch(e){ vfToast(e.message, true); }
  }

  function vfCard(p){
    if(p.error){
      return `<div class="vf-card err"><b>${vfEsc(p.label)}</b><div class="vf-reason">⚠️ ${vfEsc(p.error)} — left unchanged.</div>
...
def _verify_field_spec(kingdom):
    return _VERIFY_REGISTRY.get(kingdom, {})


def _verify_current(species, kingdom):
...
def _ai_build_verify_messages(species, kingdom, fields):
...
def ai_verify_species(kingdom, species_id, fields):
    ...
    system, user = _ai_build_verify_messages(entry, kingdom, fields)
    try:
        api_resp = _anthropic_messages(system, user, web_search=True, max_uses=5)
    except RuntimeError as e:
        return {"ok": False, "error": str(e)}
    try:
        data = _ai_parse_json(_ai_response_text(api_resp))
    except (ValueError, json.JSONDecodeError) as e:
        return {"ok": False, "error": f"Could not parse the verification: {e}",
                "raw_tail": _ai_response_text(api_resp)[-400:]}

    cur = _verify_current(entry, kingdom)
    proposals = []
    for f in fields:
        cfg = spec[f]
...
def apply_verify_decisions(kingdom, species_id, decisions):
    ...
    if applied:
        ...
        write_json_atomic(path, data)
        try:
            with open(VERIFY_AUDIT_LOG, "a", encoding="utf-8") as fh:
                for r in audit_rows:
...
def handle_api_ai_verify(params):
    ...
    if not species_id:
        return {"ok": False, "error": "missing species id"}
    if not fields:
        return {"ok": False, "error": "select at least one field to verify"}
    return ai_verify_species(kingdom, species_id, fields)
...
def handle_api_ai_verify_apply(params):
    ...
    if not species_id:
        return {"ok": False, "error": "missing species id"}
    if not decisions:
        return {"ok": False, "error": "no accepted/overridden fields to write"}
    return apply_verify_decisions(kingdom, species_id, decisions)
...
_HOUSE_RULES = (
        ...
        "don't come across as a know-it-all. The park is speaking about its own ground, so "
        "\"our\" is natural — \"our ponds\", \"the Coontie by our Butterfly Garden\" — but "
        "use it the way a person would, once in a while where it warms a sentence, never in "
        "every line. Never \"Palma Sola's ponds\" or \"their water\" as if describing "
...
_CRAFT_RULES = (
    ...
    "would hear, leave the field empty and the section will not render. Never write "
    "'essentially silent', 'no calls or vocalizations', or 'the soft rush of wings'. "
    "Writing a sentence to explain the absence of sound is padding. Do not reach for "
    "ultrasound, echolocation frequencies, or detector equipment to give the field "
    "something to say — that is laboratory detail, not what someone standing on a path "
    "can hear.\n\n"
    "SEASONALITY must not manufacture precision. Name specific months ONLY when the "
    "species is genuinely seasonal here — a migrant, or an insect with a real flight "
    "season. For a year-round species, `peak` must be null and the note should say "
    "'warm days', 'after rain', 'follows flowering' or similar. A month range invented "
    "to sound authoritative is worse than no answer, because a visitor cannot tell the "
    "difference. Never state a local peak that the sources do not support.\n\n"
    "PLANT_CONNECTIONS is the most important field on this site and the most often left "
    "empty. This is a BOTANICAL park: the reason to publish an animal at all is usually "
    "its relationship to a plant. Fill this whenever the animal has one — a larval host "
    "plant, a nectar source, a roost tree, collected plant material, or plants that "
